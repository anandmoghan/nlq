...
class DataModel:
    def __init__(self, query_list, sql_list, where_col_list, where_op_list, token_to_index, batch_size=const.BATCH_SIZE, shuffle=True):
        ...
        self.where_col_array = np.zeros([self.data_size, const.MAX_LEN])
        self.where_op_array = np.zeros([self.data_size, const.MAX_LEN])
        for idx, (query, sql, where_col, where_op) in enumerate(zip(query_list, sql_list, where_col_list, where_op_list)):
            ...
            if sql['sel'] < 22:
                ...
                self.where_col_array[idx, const.MAX_LEN - len(encoded_query) + 1:-1] = where_col
                self.where_op_array[idx, const.MAX_LEN - len(encoded_query) + 1:-1] = where_op
    ...
    def reset_pointer(self):
        ...
        if self.shuffle:
            ...
            self.where_col_array = self.where_col_array[shuffle_idx, ]
            self.where_op_array = self.where_op_array[shuffle_idx, ]
    ...
    def next_batch(self):
        ...
        if self.current_batch > self.total_batches:
            self.reset_pointer()
        return self.encoded_query_matrix[batch_idx, ], self.aggregate_array[batch_idx, ], self.select_array[batch_idx, ], self.where_col_array[batch_idx, ], self.where_op_array[batch_idx, ]
