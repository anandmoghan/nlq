...
from models.predictors import AggregatePredictor, SelectPredictor
...
class NLQModel:
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=GPU_OPTIONS)) as sess:
            ...
            for e in range(args.epochs):
                ...
                for b in range(num_batches):
                    ...
                    print("{}/{} : Epoch {} - Loss = {:.3f}, Accuracy = {:.2f}".format(e * num_batches + b + 1, args.epochs * num_batches, e + 1, train_loss, accuracy))
                    if (e * num_batches + b) % args.save_every == 0 or ( e == args.epochs - 1 and b == data_model.total_batches - 1):
    ...
    def start_train(self, data_model, validation_model):
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=GPU_OPTIONS)) as sess:
            ...
            for e in range(args.epochs):
                logger.start_timer('Epoch: {:d}'.format(e+1))
                sess.run(tf.assign(select_predictor.lr, args.lr * (args.decay ** e)))
                train_accuracy = 0
                for b in range(num_batches):
                    state = sess.run(select_predictor.initial_state)
                    query, _, true_select = data_model.next_batch()
                    feed = {
                    ...
                    for i, (c, h) in enumerate(select_predictor.initial_state):
                        feed[c] = state[i].c
                        feed[h] = state[i].h
                    _, train_loss, state, predicted_output = sess.run([select_predictor.optimizer, select_predictor.cost, select_predictor.final_state, select_predictor.predicted_output], feed)
                    accuracy = 100 * accuracy_score(true_select, predicted_output)
                    train_accuracy += accuracy
                    print("{}/{} : Epoch {} - Loss = {:.3f}, Accuracy = {:.2f}".format(e * num_batches + b + 1, args.epochs * num_batches, e+1, train_loss, accuracy))
                    if (e * num_batches + b) % args.save_every == 0 or ( e == args.epochs - 1 and b == data_model.total_batches - 1):
                        checkpoint_path = os.path.join(args.save, 'model.ckpt')
                ...
                for b in range(validation_model.total_batches):
                    ...
                    for i, (c, h) in enumerate(select_predictor.initial_state):
                        ...
                        feed[h] = state[i].h
                    _, train_loss, state, predicted_output = sess.run(
                        [select_predictor.optimizer, select_predictor.cost, select_predictor.final_state,
                         select_predictor.predicted_output], feed)
                    validation_accuracy += 100 * accuracy_score(true_select, predicted_output)
                logger.end_timer()
                print('Train Accuracy = %.2f, Validation Accuracy = %.2f' % (train_accuracy/data_model.total_batches, validation_accuracy/validation_model.total_batches))
                '''